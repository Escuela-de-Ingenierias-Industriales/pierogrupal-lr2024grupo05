...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) Sensor_bateria_M->Timing.clockTick0;
  Sensor_bateria_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Sensor_bateria_M, 0);
  ...
  Sensor_bateria_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Sensor_bateria_M->extModeInfo, &rtmGetTFinal
    (Sensor_bateria_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Sensor_bateria_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Sensor_bateria_M);
